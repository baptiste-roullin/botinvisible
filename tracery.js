...
var tracery = function() {
	...
	TraceryNode.prototype.expand = function(preventRecursion) {
		...
		if (this.raw == "nom_masc") {
      
				storage.setItemSync("gender", "masc")

			}
			else if (this.raw == "nom_fem") {
				storage.setItemSync("gender", "fem")
	...
	function RuleSet(grammar, raw, key) {
		this.raw = raw;
	...
	RuleSet.prototype.selectRule = function(errors) {
		...
		if (this.defaultRules !== undefined) {
			...
			if (this.key == "adjectif") {

				if 	(storage.getItemSync("gender") == "masc" ) {
					return this.defaultRules[0];
				}
				else if (storage.getItemSync("gender") == "fem") {
					return this.defaultRules[1];
				}
		}
	...
	var Symbol = function(grammar, key, rawRules) {
		...
		this.baseRules = new RuleSet(this.grammar, rawRules, this.key);
		this.clearState();
